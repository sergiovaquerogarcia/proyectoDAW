<?php 
    ...
    if(isset($_REQUEST["cusumodificar"])) {
        $cusumodificar= $_REQUEST["cusumodificar"];    
    }
    ...
    if (isset($_REQUEST["cusumodificar"])) {
        $cusumodficar = $_REQUEST["cusumodificar"];
        $cnn = conectar_db();
        
        $stmt =$cnn->prepare("SELECT * FROM usuarios WHERE codUsuario = :codUsuario");
        $rows = $stmt->execute(array(":codUsuario"=>$cusumodificar));
        $stmt->setFetchMode(PDO::FETCH_CLASS, 'usuario');

        if ($rows == 1) {
            $fila = $stmt->fetch();
            $tipoUsuarioModificar = $fila->getTipoUsuario();
            $tipoUsuarioModificar1 = $tipoUsuarioModificar;
            $dni = $fila->getDni();
            $clave = $fila->getClave();
    ...
    if (isset($_REQUEST["cusumodificar1"]) == TRUE) {
        $cusumodificar = $_REQUEST["cusumodificar1"];

        if(isset($tipoUsuarioModificar) == FALSE) {
            $clave = $_REQUEST["clave"];
        }
       
        if(empty($_REQUEST["tipoUsuarioModificar"])==FALSE) {
            $tipoUsuarioModificar = $_REQUEST["tipoUsuarioModificar"];
        }
        else {
            $tipoUsuarioModificar = $tipoUsuarioModificar1;
        }
       ...
       /* if ($tipoUsuario != 1 && $tipoUsuario != 2 && $tipoUsuario != 3) {
            $error [9]= "<p class='error'>ERROR. *Tipo Usuario Incorrecto. Elija: usuario), 2 (Trabajador) o 3 (administrador).</p>";
            $codigo10 = 10;
        }
       ...
       if (!isset($codigo1) && !isset($codigo2) && !isset($codigo3) && !isset($codigo4)  && !isset($codigo5)  && !isset($codigo6)  && !isset($codigo7)  && !isset($codigo8) && !isset($codigo9)) {
            //echo "ACTIVO: " . $activo;
            updateUsuario ($cusumodificar, $tipo, $cp, $tipoUsuarioModificar, $clave, strtoupper($dni), strtoupper($nombre), strtoupper($direccion), strtoupper($poblacion), strtoupper($provincia), $telefono, strtolower($email), $activo);
        }
    }
?>
<html lang="es">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                <h2 class="h3 text-center text-warning">EDITAR USUARIO</h2><hr><br>
                    <form method="post" action="">
                        <input type="hidden" name="cusumodificar1" value="<?php echo $cusumodificar?>">
                        ...
                        ?>
                            <div class="form-group">
                                <label for="tipoUsuarioModificar" class="col-sm-12 col-form-label text-warning"><h5>TIPO DE USUARIO *</h5></label>
                                <div class="col-sm-12">
                                    <select class="form-control" id="tipoUsuarioModificar" name="tipoUsuarioModificar" required>
                                        <?php
                                        if (isset($tipoUsuarioModificar) && ($tipoUsuarioModificar == 1)) {
                                        ?>
                                        ...
                                        if (isset($tipoUsuarioModificar) && ($tipoUsuarioModificar == 2)) {
                                        ?>
                                        ...
                                        if (isset($tipoUsuarioModificar) && ($tipoUsuarioModificar == 3)) {
                                        ?>
                            ...
                            <div class="form-group">
                                <label for="activo" class="col-sm-12 col-form-label text-warning"><h5>USUARIO ACTIVADO / DESACTIVADO *</h5></label>
                                <div class="col-sm-12">

<?php
    ...
    if(isset($_REQUEST["cusudelete"])) {
        $cusudelete = $_REQUEST["cusudelete"];    
    }
    
    if(empty($_REQUEST["cusudelete"]) == TRUE) {
        ...
        $rows = $stmt->execute(array(":codUsuario"=>$cusudelete));
    ...
    if(isset($_REQUEST["eliminar"]) == TRUE) {
        ...
        //$stmt =$cnn->prepare("Delete from usuario where dni = :dni");
        $stmt =$cnn->prepare("update usuarios SET activo= :activo where codUsuario= :codUsuario");
        $rows = $stmt->execute(array(":activo"=>$activo, ":codUsuario"=>$cusudelete));
        if ($rows == 1) {       
            if ($tipo == 1) {
                header("location:/beautyandshop/admin/zonaUsuarios.php?nombreusuario= " . $nombre);
            }
            /*else {
                session_start();
                $_SESSION = array(); 
                session_destroy();
                header("location:/tiendaPeluqueria/index.php?nombreusuario= " . $nombre); 
            }*/
        }
        else {
...
<html lang="es">
<head>
    <title>Borrar Usuario. Beauty And Shop</title>
    <meta charset="utf-8">
    ...
    <section class="container-fluid">
       <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                ...
                <h4 class="h4 text-center text-danger">¿Está seguro de eliminar el siguiente usuario?</h4>
                <div class="text-center">
                    <h5 class="h5 text-info">Nombre Usuario:</h5> <span><strong><?php echo $nombre;?></strong></span>
                    <h5 class="h5 text-info">Código:</h5> <span><strong><?php echo $cusudelete;?></strong></span><br>
                </div><br><br><hr>

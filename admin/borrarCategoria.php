<?php
    ...
    if(empty($_REQUEST["codigo"]) == TRUE) {
        ...
        $rows = $stmt->execute(array(":codigo"=>$codigoCategoria));
    ...
    if(isset($_REQUEST["eliminar"]) == TRUE) {
        ...
        $stmt =$cnn->prepare("UPDATE categorias SET activo= :activo WHERE codigo= :codigo");
        $rows = $stmt->execute(array(":activo"=>$activo, ":codigo"=>$codigoCategoria));
        if ($rows == 1) {       
            if ($tipoUsuario == 1 || $tipo == 1) {
                header("location: /beautyandshop/admin/zonaCategorias.php?nombreCategoria= " . $nombre);
            }
...
<html lang="es">
    <head>
        <title>Borrar Categoria. Beauty And Shop</title>
        <meta charset="utf-8">
    ...
    <section class="container-fluid">
       <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                <h2 class="h2 text-center text-warning">BORRAR CATEGORIA</h2><hr><br>
                <h4 class="h4 text-center text-danger">¿Está seguro de eliminar la siguiente Categoría?</h4>
                <div class="text-center">
                    <h5 class="h5 text-info">Nombre Categoria:</h5> <span><strong><?php echo $nombre;?></strong></span>
                    <h5 class="h5 text-info">Código:</h5> <span><strong><?php echo $codigoCategoria;?></strong></span><br>
                </div><br><br><hr>

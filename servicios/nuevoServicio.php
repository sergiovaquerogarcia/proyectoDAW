<?php
    ...
    if(isset($_REQUEST["enviar"]) == TRUE) {  
        ...
        if (isset($_REQUEST['descuento'])) {
            if(empty(isset($_REQUEST['descuento']))) {
                $descuento = 0.0;
            }
            else {
        ...
        /* BUSCO SI YA EXISTE LA DESCRIPCIÓN DEL SERVICIO*/
        $cnn = conectar_db();
                
        $datosServicios = [];

        $enc = 0;
        $datosServicios = llenarArrayServicios(0);

        for($i=0;$i<count($datosServicios) && $enc == 0;$i++) {
            $descripcionAux = $datosServicios[$i]-> getDescripcion();
            if($descripcionAux == $descripcion) {
                $enc = 1;
            }
        }

        if ($enc == 1) {
            $error [3] = "<div class='smsError'><b>ERROR!!! EL SERVICIO INTRODUCIDO YA EXISTE.</b></div><br><br>";
            $codigo3 = 3;     
        }
        else {       
            addServicio ($tipo, strtoupper($descripcion), $precio, $descuento, $duracionServicio, $codFamilia, $activo);
        }
...
<html lang="es">
    <head>
        <title>Nuevo SERVICIO. Beauty And Shop</title>
        <meta charset="utf-8">
    ...
    <section class="container-fluid">
        <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                ...
                ?>
                <h2 class="h3 text-center text-warning">NUEVO SERVICIO</h2><hr><br>
                    <form name="nuevoProducto" method="post" action="">
                        <!--<div class="form-group">
                            <label for="codigo" class="col-sm-12 col-form-label text-warning"><h5>CÓDIGO SERVICIO</h5></label>
                            <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <input class="form-control errorImput" type="text" name="descripcion" id="descripcion" maxlength="90" required value="<?php if(isset($descripcion)) echo $descripcion ?>">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <input class="form-control errorImput" name="duracionServicio" id="duracionServicio" required type="number" step="5" min="0" max="9999" size="50" value="<?php if(isset($duracionServicio)) echo $duracionServicio ?>">
                            </div>
                        </div>

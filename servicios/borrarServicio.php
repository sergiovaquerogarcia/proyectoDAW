<?php
    ...
    if(empty($_REQUEST["codigo"]) == TRUE) {
        ...
        $rows = $stmt->execute(array(":codServicio"=>$codServicio));
    ...
    if(isset($_REQUEST["eliminar"]) == TRUE) {
        ...
        $stmt =$cnn->prepare("UPDATE servicios SET activo= :activo WHERE codServicio= :codServicio");
        $rows = $stmt->execute(array(":activo"=>$activo, ":codServicio"=>$codServicio));
        if ($rows == 1) {       
            if ($tipoUsuario == 1 || $tipo == 1) {
                header ("location:/beautyandshop/servicios/zonaServicios.php?desc= " . $descripcion);
            }
...
<html lang="es">
    <head>
        <title>Borrar SERVICIO. Beauty And Shop</title>
        <meta charset="utf-8">
...
<body>
    ...
    <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                <h2 class="h2 text-center text-warning">BORRAR SERVICIO</h2><hr><br>
                <h4 class="h4 text-center text-danger">¿Está seguro de eliminar el siguiente Servicio?</h4>
            
                <div class="text-center">
                    <h5 class="h5 text-info">Descripción Servicio:</h5> <span><strong><?php echo $descripcion;?></strong></span>
                    <h5 class="h5 text-info">Código:</h5> <span><strong><?php echo $codServicio;?></strong></span><br>
                </div><br><br><hr>

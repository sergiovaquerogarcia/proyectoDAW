<?php
    ...
    if(isset($_REQUEST["enviar"]) == TRUE) {
        $nombreFamilia = strtoupper($_REQUEST["nombreFamilia"]);
        $activoFamilia = $_REQUEST["activoFamilia"];
                
        /*if (isset($_REQUEST["categoriaPadre"])) {
        ...
        if (empty($_REQUEST["nombreFamilia"]) == TRUE) {
            $error [0]= "<p class='error'>ERROR. *El NOMBRE de la FAMILIA no puede estar en blanco.</p>";
            $codigo0 = 0;
        }
        else {
            $datosFamilias = [];
            ...
            $enc = 0;
            $datosFamilias = llenarArrayFamilias(0);
                        
            for($i=0;$i<count($datosFamilias) && $enc == 0;$i++) {
                $nombreFamiliaAux = $datosFamilias[$i]-> getNombre();
                if($nombreFamiliaAux == $nombreFamilia) {
                    $encontrado = 1;
                    $error [0]= "<p class='error'>ERROR. *LA FAMILIA YA EXISTE.</p>";
                    $codigo0 = 0;
                }
            }
        }
        ...
        if (!isset($codigo0) && !isset($codigo1)) {
            addFamilia (strtoupper($nombreFamilia), $activoFamilia);
        }
    }
?>
<html lang="es">
    <head>
        <title>Nueva FAMILIA. Beauty And Shop</title>
        <meta charset="utf-8">
    ...
    <section class="container-fluid">
        <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                <h2 class="h3 text-center text-warning">NUEVA FAMILIA</h2><hr><br>
                <form method="post" action="">
                    <div class="form-group">
                        <label for="nombreFamilia" class="col-sm-12 col-form-label text-warning"><h5>NOMBRE FAMILIA</h5></label>
                        <div class="col-sm-12">
                           <input class="form-control errorImput" type="text" required maxlength="80" size="80" id="nombreFamilia" name="nombreFamilia" value="<?php if(isset($nombreFamilia)) echo $nombreFamilia ?>" <?php if(isset($codigo0) && $codigo0 == 0){ echo "autofocus"; } ?>>
                        </div>
                    </div>
                        
                    <!--MUESTRO UN ERROR SI NO ES CORRECTO EL NOMBRE DE LA CATEGORIA O SI ESTÁ REPETIDO.-->
                    <?php
                    ...
                    <!--MUESTRO UN ERROR SI NO ES CORRECTO EL CAMPO ACTIVA.-->
                    <?php
                    ...
                    <div class="text-center">
                        ...
                        <button class="btn btn-warning text-center" type="reset">LIMPIAR</button>

...
<html lang="es">
    <head>
        <title>Mis CITAS. Beauty And Shop</title>
        <meta charset="utf-8">
    ...
    <section class="container-fluid">
        <div class="row">        
            ...
            <div class="col-sm-12 col-md-12 col-lg-8">
                ...
                // LLENO EL ARRAY CON TODAS LAS CITAS ACTIVAS.
                $datosCitas = [];
                $datosCitas = llenarArrayCitasTodas(2);

                // AHORA VOY A SACAR SÓLO LAS CITAS DEL USUARIO.
                $datosMisCitas = [];
                $i=0;
                $j=0;
                for($i=0;$i<count($datosCitas);$i++) {
                    if ($datosCitas[$i]->getCodUsuario() == $cusu) {
                        $datosMisCitas[$j] = new cita();
                        $datosMisCitas[$j] -> setCodCita($datosCitas[$i]->getCodCita());
                        $datosMisCitas[$j] -> setCodUsuario($datosCitas[$i]->getCodUsuario());
                        $datosMisCitas[$j] -> setFechaCita($datosCitas[$i]->getFechaCita());
                        $datosMisCitas[$j] -> setHoraCita($datosCitas[$i]->getHoraCita());
                        $datosMisCitas[$j] -> setTotal($datosCitas[$i]->getTotal());
                        $j++;
                    }
                }
                echo mostrarTodasMisCitas($datosMisCitas);
                ?>
                <br>
            </div>
